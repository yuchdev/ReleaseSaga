...
- The cross-cutting rules in `@docs/dev/python_coding_standard.md` - full annotations, ruff,
  ...
  Also name `CLAUDE.md` and the specific section the subtask needs: **`## Commands`** for the
  three conventions that silently break agents here - tests live in **`test/`, not `tests/`**;
  the repo-root `ruff.toml` takes precedence over `[tool.ruff]` in `pyproject.toml` (and its
  `extend-exclude` entries reference paths that do not exist in this repo, so they are inert);
  and `pyproject.toml` sets `pythonpath = ["src"]`, so `release_saga` imports without an
  editable install. **`## What this is`** for the target-project boundary - this tool reads and
  mutates *another* project's `pyproject.toml` and `dist/`, never its own - which any subtask
  touching paths, globs, or config must hold. **`## Architecture`** for the rollback and
  `check()` invariants that constrain every change under `src/release_saga/steps/` and
  `pipeline.py`. Note there is **no log-redaction utility**: logging is plain `print()` to
  stderr via `pipeline._log()` plus bare `print()` in `cli.py` and
  `steps/github_release.py`, so redaction is the author's responsibility at each call site.
  If the milestone's `plan.md` carries its own cross-cutting
...
| Condition                                                | Skill                                                                                                                                |
|----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|
| Always                                                   | `/test-gap src/release_saga/<subtask-module>/` - report coverage delta; if < 85 % delegate missing tests to `testing-expert`. |
| `pyproject.toml` or any `requirements*.txt` changed      | `/dep-audit`                                                                                                                         |
| Any file under an API route, auth, or middleware path changed, or anywhere handling credentials/tokens | `/secret-scan src/release_saga/<changed-module>/` |
...
- **Task complete** → this is the **one** place the whole suite runs: execute
  `uv run pytest tests/unit/ -q --cov=release_saga` once (summary line only), then run
