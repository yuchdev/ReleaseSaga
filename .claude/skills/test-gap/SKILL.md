...
1. Establish current coverage:
   `uv run pytest tests/unit/ -q --cov=release_saga --cov-report=term-missing`
   (scope with `--cov=release_saga.<subpkg>` when a path is given).
2. Spawn the **`testing-expert`** agent with the coverage output and the target path. Ask it
   ...
   (blast-radius criteria + RegeaseSaga module examples per tier):
