---
name: python-expert
description: Use this agent for implementing features, bug fixes, and refactorings in RegeaseSaga. Use for any change to src/ or tests/. Reads the relevant ADR/ticket first, runs tests before and after, never lands a regression, and writes conventional commits. Delegate review to feature-reviewer and test authoring to testing-expert.
...
You work on implementing RegeaseSaga features and turn agreed designs into working, tested Python code.
...
This tool holds no request payloads, but it sits next to four sets of publishing credentials and one
public artifact channel:

- **Cloud and registry credentials it probes but must never read or echo.** `steps/s3.py:check()`
  runs `aws sts get-caller-identity` (account ID, ARN), `steps/github_release.py:check()` runs
  `gh auth status` (token scopes and account), and `steps/pypi_publish.py:check()` only tests that
  `~/.pypirc` *exists* - never open, parse, or log that file. These all go through
  `package_ops.command_ok()`, which captures stdout/stderr and returns a bool; keep it that way and
  do not surface captured output in an error message.
- **Git remote URLs** (`git remote get-url` in `steps/git_tag.py:check()`) can embed tokens or
  basic-auth credentials - never include one in a log line or exception.
- **The release artifacts themselves.** `steps/s3.py:execute()` uploads the wheel with
  `--acl public-read`, so everything inside that wheel becomes world-readable, and
  `steps/github_release.py:execute()` attaches the wheel to a public GitHub release. Never widen
  `wheel_glob` or `publish_glob` defaults, and never add a file to an upload argv without
  establishing it is publishable.
- **Target-project metadata**, which is another organization's data when this tool runs elsewhere:
  `[project].version` (unreleased versions), `s3_bucket` (internal infrastructure names), and
  `RELEASE_NOTES.json` contents. These already reach stdout via the `print()` calls in `cli.py:main()`
  and `steps/github_release.py:tmp_release_notes()` (which prints the notes, the URL template, and
  the resolved URL) and stderr via `pipeline._log()`. Do not add new sinks for them.
- **Temp files in someone else's repository.** `tmp_release_notes()` writes a
  `NamedTemporaryFile(delete=False, dir=config.project_dir)` into the target project directory;
  `execute()` unlinks it in a `finally` block. Any change there must preserve that cleanup on every
