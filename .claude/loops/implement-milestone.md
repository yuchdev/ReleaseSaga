...
**ReleaseSaga defines no exit gates beyond "full suite green + `/pr-review`." Do not invent any.**
The complete bar is what `.github/workflows/ci.yml` enforces on every push and PR to `master`:

1. `uv run ruff check .` - note that the repo-root `ruff.toml` takes precedence over
   `[tool.ruff]` in `pyproject.toml`, so `ruff.toml` is the config actually in effect.
2. `uv run pytest` - the tests live in **`test/`**, not `tests/`. `pyproject.toml` sets
   `pythonpath = ["src"]`, so no editable install is needed to run them.

There is **no coverage floor** (nothing configures `--cov-fail-under`, though `pytest-cov` is
available for reporting), **no integration or e2e suite**, no benchmark or ground-truth run, and
no staging deployment. There is also no `docs/roadmap/` in this repository yet, so a milestone's
gates come from its own `plan.md` or from this list - never from a presumed convention.

If a future `plan.md` adds a gate, record it in `research_digest.exit_gates` from R1 rather than
hard-coding it here.
...
**Where ReleaseSaga records ratified divergence:** the default above stands - the milestone
`status.md` `## Notes & decisions` section. This repository has no `docs/adr/` directory (no `docs/`
tree at all yet), so no ADR is required for a contract-level change, and there is no sign-off
authority beyond the `AskUserQuestion` ruling in step 3.

One addition specific to this project: the *only* current records of architecture are the
`## Architecture` sections of `CLAUDE.md` and `README.md`, plus `README.md`'s `## Release steps`
table and `## Extending with custom steps` section. A ratified divergence that changes the
`ReleaseStep` contract (`check()`/`execute()`/`rollback()`), the rollback ordering in
`pipeline.py`, the `ReleaseConfig` fields, or the config precedence rule **must** update both files
in the same task - otherwise the ruling is recorded only in a status file while the documents
agents read on every session still describe the superseded design. Note also that `README.md`
already carries two `<!-- Milestone 0004, Task 04.0 -->` placeholders (the competitive-comparison
and migration sections), so README edits are legitimately milestone-tracked work here.
...
ReleaseSaga does not raise the bar above the `implement-subtasks.md` defaults: a task and a
milestone are both complete at **full suite green + `/pr-review` LGTM**, where "full suite" is
`uv run ruff check .` followed by `uv run pytest` over the `test/` directory - the same two
commands `.github/workflows/ci.yml` runs. No minimum coverage percentage is enforced, there is no
integration suite to run, and nothing is deployed. The M4 and M5 gate lists above are therefore
complete as written.
