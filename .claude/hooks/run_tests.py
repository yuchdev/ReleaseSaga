...
Runs, in order, the same checks CI expects a clean session to satisfy:
  ...
  3. ``uv run pytest -q --cov=release_saga --cov-report=term-missing``
...
Skip the gate for a known-WIP session by exporting ``RELEASE_SAGA_SKIP_TESTS=1``.
"""
...
def main():
    ...
    if event.get("stop_hook_active"):
        allow()
    if os.environ.get("RELEASE_SAGA_SKIP_TESTS") == "1":
        append_log("tests.log", "SKIPPED (RELEASE_SAGA_SKIP_TESTS=1)")
        allow()

    steps = [
        ("ruff --fix", ["uv", "run", "ruff", "check", ".", "--fix"]),
        ("ruff check", ["uv", "run", "ruff", "check", "."]),
        ("pytest+cov", ["uv", "run", "pytest", "-q", "--cov=release_saga", "--cov-report=term-missing"]),
    ]
    ...
    for label, cmd in steps:
        ...
        if proc.returncode != 0:
            ...
            block(
                f"Stop blocked by RegeaseSaga run-tests: `{' '.join(cmd)}` failed.\n"
                f"{_tail(proc)}\n"
                ...
                "guessing. Export RELEASE_SAGA_SKIP_TESTS=1 only if this is intentional WIP."
            )
